...
# def get_eo_paper_counts(
#     df_payloads, 
#     api_key,
#     limit_per_request=100,      # max per API request
#     pause_sec=1.1,              # MUST be >1.0 due to 1 req/sec limit
#     max_retries=3,
#     plot=False
# ):
#     """
#     Fetches number of papers mentioning each EO satellite.
#     Uses Semantic Scholar API key (1 req/sec rate limit).
#     Supports pagination up to 1000 papers per satellite.
...
#             query_string = f'"{sat}" satellite'
#             params = {
#                 "query": query_string,
#                 "limit": min(limit_per_request, 1000 - offset),
#                 "offset": offset,
#                 "fields": "title"
#             }
...
#                         print(f"❌ Too many retries for {sat}. Giving up.")
#                         break
#                     print(f"⚠️ Rate limit hit for {sat}. Retry {retries}/{max_retries} after {wait:.1f}s...")
#                     time.sleep(wait)
#                     wait *= 1.5
#                     continue
#                 else:
#                     print(f"❌ Error {response.status_code} for {sat}")
#                     break
...
#         raw_counts[sat] = total_papers
#         print(f" - {sat}: {total_papers} papers")

#     # Normalize weights
#     total = sum(raw_counts.values())
#     if total == 0:
#         normalized_weights = {k: 1/len(raw_counts) for k in raw_counts}  # fallback
#     else:
#         normalized_weights = {k: v/total for k, v in raw_counts.items()}

#     if plot:
#         plt.figure(figsize=(12,5))
#         plt.bar(raw_counts.keys(), raw_counts.values())
#         plt.xticks(rotation=90)
#         plt.title("Number of Papers Mentioning Each EO Satellite")
