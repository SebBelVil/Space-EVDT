...
def remove_duplicate_cdms(df_CDM):
    ...
    Parameters:
        df_CDM (pd.DataFrame): The input CDM dataframe.
    
    Returns:
        pd.DataFrame: CDM dataframe with unique pairs, sorted by MAX_PROB.
    """
...
def filter_gp(df_gp):
    ...
    -----------
    df_gp : pd.DataFrame
        The full gp dataframe.

    Returns:
    --------
    pd.DataFrame
        Filtered gp dataframe with only active satellites and relevant columns.
    """
